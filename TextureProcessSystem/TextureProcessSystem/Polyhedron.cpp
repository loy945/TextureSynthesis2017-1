...
void Polyhedron::mark()
{
	...
	for (int i = 0; i < this->numberF; i++)
	{
		...
		for (int j = 0; j < 3; j++)
		{
			x[j] = this->pU[this->Face[i][j]];
			y[j] = this->pV[this->Face[i][j]];

			float xmin = (rect[0]->x>rect[1]->x) ? rect[1]->x : rect[0]->x;
			float xmax = (rect[0]->x>rect[1]->x) ? rect[0]->x : rect[1]->x;

			float ymin = (rect[0]->y>rect[1]->y) ? rect[1]->y : rect[0]->y;
			float ymax = (rect[0]->y>rect[1]->y) ? rect[0]->y : rect[1]->y;

			if (x[j]<xmin || x[j]>xmax || y[j]<ymin || y[j]>ymax)
			{
				inRect = false;
			}
		}
